...
function Header() {
  ...
  return (
    <div className="relative h-[484px] flex flex-col justify-center items-center">
      <div className="fixed top-0 z-99 w-full flex justify-between items-center p-4 max-h-fit sm:max-h-[100px] gap-4">
        ...
        {menuOpen && (
          <div className="fixed top-0 left-0 w-3/4 h-full bg-white shadow-lg z-50 p-4 transition-transform duration-300 md:hidden">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold text-[#BB1D1D]">Menu</h2>
              <X
        ...
        <div className="hidden md:flex items-center gap-10 sm:gap-7 md:gap-4 lg:gap-10 w-full sm:w-auto">
          {headings.map((heading, index) => (
            <p
              key={index}
              className={`text-md md:text-md leading-[18px] cursor-pointer text-nowrap font-bold font-manrope ${
                selectedIndex === index
                  ? "text-[#BB1D1D]"
                  : "text-[#272D2F]"
              }`}
        ...
        {/* Buttons */}
        <div className="flex justify-end gap-2 sm:gap-4 md:gap-1 lg:gap-2 w-full sm:w-auto">
      ...
      <div className="text-center px-4">
        <h1 className="font-extrabold text-[28px] leading-[32px] sm:text-[36px] sm:leading-[40px] md:text-[48px] md:leading-[44px] lg:text-[54px] lg:leading-[48px] pb-6 sm:pb-8 lg:pb-10">
          Home {">"} Host
        </h1>
        <p className="font-bold text-[16px] leading-[22px] sm:text-[18px] sm:leading-[24px] md:text-[20px] md:leading-[26px] lg:text-[22px] lg:leading-[28px]">
